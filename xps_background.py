...
from arpes.utilities.conversion import convert_to_xarray
...
class xps_BackgroundWindow(QtWidgets.QDialog):
	...
	def get_background(self):
		self.background_type = self.background_combo.currentText()
		self.background = self.set_background(self.data,self.background_type)
		self.close()
		return

	def set_background(self,data,background_type):
		...
		#function will return 1 if bacground tyoe is not recognized
		if background_type == "None":
			return None
		elif background_type == "shirley":
			background = background_calculation.shirley_calculate(data[:,0],data[:,1])
		elif background_type == "tougaard":
			background = background_calculation.tougaard_calculate(data[:,0],data[:,1])
		else:
			print("background type not recognised")
			background = 1
		return background
...
[docs]@update_provenance("Remove Shirley background")
...
[docs]@update_provenance("Calculate limited range Shirley background")
