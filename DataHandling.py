...
class XPS_DataRangeWindow(QtWidgets.QDialog):
    ...
    def data_range(self):
         self.span = SpanSelector(self.MplWidget.canvas.axes, self.onselect, 'horizontal',
                                        useblit=True, interactive=True, drag_from_anywhere=True, rectprops=dict(alpha=0.5, facecolor='red'))
         return
...
class XPS_PeakFindingWindow(QtWidgets.QDialog):
    ...
    def findpeaks(self):
        if (str(self.peak_search_parameters["height"]).isdigit() == False and self.peak_search_parameters["height"] != None) or (str(self.peak_search_parameters["distance"]).replace('.','',1).isdigit() == False and self.peak_search_parameters["distance"] != None) or (str(self.peak_search_parameters["prominence"]).replace('.','',1).isdigit() == False and self.peak_search_parameters["prominence"] != None) or (str(self.peak_search_parameters["width"]).isdigit() == False and self.peak_search_parameters["width"] != None):
            self.warning_label.setVisible(True)
            return
        else:
            self.inital_peak_info = sps.find_peaks(self.data[:,1], height = self.peak_search_parameters["height"], distance = self.peak_search_parameters["distance"], prominence = self.peak_search_parameters["prominence"], width = self.peak_search_parameters["width"])
            data_handling.set_peak_data(self,self.data,self.peak_search_parameters,self.inital_peak_info)
            self.peak_no = len(self.inital_peak_info[0])
...
class XPS_PeakSettingWindow(QtWidgets.QDialog):
    ...
    def __init__(self):
        ...
        self.data = None
        self.peak_no = 1
        self.inital_peak_info = [np.array([None]),{}]
        self.initUI()
    ...
    def item_check(self, item):
        ...
        if item is None:
            pass
        elif item.text().replace(".","",1).isdigit() == True:
            if (float(item.text()) >= self.data[0,0]) & (float(item.text()) <= self.data[-1,0]):
                self.set_inital_peak_info_length()
                self.set_inital_peak_info_values()
            else:
    ...
    def set_inital_peak_info_length(self):
        if len(self.inital_peak_info[0]) > self.peak_set_table.rowCount():
            self.inital_peak_info[0] = np.delete(self.inital_peak_info[0],np.where(self.inital_peak_info[0] == 0))
        elif len(self.inital_peak_info[0]) < self.peak_set_table.rowCount():
            self.inital_peak_info[0] = np.append(self.inital_peak_info[0],np.zeros(self.peak_set_table.rowCount() - len(self.inital_peak_info[0])))
        else:
            pass
        return
    
    def set_inital_peak_info_values(self):
        for i in range(self.peak_set_table.rowCount()):
            if self.peak_set_table.item(i,0) is not None:
                self.inital_peak_info[0][i] = np.abs(self.data[:,0] - float(self.peak_set_table.item(i,0).text())).argmin()
            else:
                pass
        return
    ...
    def graph_data_basic_plus_points(self):
        ...
        self.MplWidget.canvas.axes.clear()
        self.MplWidget.canvas.axes.plot(X,Y)
        if np.any(self.inital_peak_info[0]) is None:
            pass
        else:
            print(self.inital_peak_info[0])
            for i in range(len(self.inital_peak_info[0])):
                self.MplWidget.canvas.axes.plot(X[int(self.inital_peak_info[0][i])],Y[int(self.inital_peak_info[0][i])],'x',color = "green")
        #draws
        self.MplWidget.canvas.draw()
        return
...
class data_handling():
    ...
    def graph_data(self):
        ...
        if self.inital_peak_info is None or self.show_peakposotions_button.isChecked() == False:
            pass
        else:
            for i in range(len(self.inital_peak_info[0])):
                self.MplWidget.canvas.axes.plot(X[self.inital_peak_info[0][i]],Y[self.inital_peak_info[0][i]],'x',color = "green")
        ...
        if self.fit_results is None or self.show_fittingcurve_button.isChecked() == False:
            ...
            self.plot_fitting(self.data[self.gind[0]:self.gind[1],0],self.fit_results,self.background[self.gind[0]:self.gind[1]])
        ...
        if self.fit_results is None or self.show_fittedpeaks_button.isChecked() == False:
            ...
            self.plot_peak_model(self.data[self.gind[0]:self.gind[1],0],self.fit_results,self.peak_no,self.background[self.gind[0]:self.gind[1]])
        #draws
        self.MplWidget.canvas.draw()
        return
    ...
    def set_peak_data(self,data,peaks,peakx):
        ...
        self.peak_text = "Number of peaks detected is: ".format(len(peakx[0]))
        self.peak_text = self.peak_text + "\nPeak data is as follows:"
        for i in range(len(peakx[0])):
            self.peak_text = self.peak_text + "\npeak "+ str(i+1) + ": position = "+ str(data[peakx[0][i],0]) + ", height = " + str(data[peakx[0][i],1])
            if peaks["prominence"] is None:
                pass
            else:
                self.peak_text = self.peak_text + "\nleft base = " + str(data[peakx[1]["left_bases"][i],0]) + ", right base = " + str(data[peakx[1]["right_bases"][i],0])
        return
